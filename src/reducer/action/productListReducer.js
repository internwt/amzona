...
export const itemLists = () => async (dispatch) => {
    try {
        ...
        const respose = await Axios.get('http://localhost:5000/products/items')
...
export const detailsProduct = (id) => async (dispatch) => {
    try {
        ...
        const respose = await Axios.get(`http://localhost:5000/products/details/${id}`)
        
        dispatch({ type: actionTypes.PRODUCT_DETAIL_SUCCESSS, payload: respose.data })
    } catch (error) {
        console.log(`erooore`,error.message)
        dispatch({ type: actionTypes.PRODUCT_DETAIL_FAILURE,payload:error.message })
    }
}